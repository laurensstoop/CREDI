...
# we want to see all years
for seasonday in np.arange(212):
    
    # Show the data for all the years
    if seasonday == 1:
        ds_subsW[str(year)] = ds_WONanom.anom.sel(time=slice(str(year)+'-09-01' + timedelta(days=int(seasonday)), str(year+1)+'-03-31')+ timedelta(days=int(seasonday))).cumsum().to_numpy()
    else:
        ds_subsW[str(year)] = ds_WONanom.anom.sel(time=slice(str(year)+'-09-01'+ timedelta(days=int(seasonday)), str(year+1)+'-03-31')+ timedelta(days=int(seasonday))).cumsum().values
