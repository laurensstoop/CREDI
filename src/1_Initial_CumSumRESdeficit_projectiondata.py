...
REGION = 'NL01' # NL01, DE01, DKE1, FR01, NOM1, UK01
...
dsr_hist = ds_hist[REGION].rolling(time=1008,center=True).mean()
dsr_proj = ds_proj[REGION].rolling(time=1008,center=True).mean()
...
ds_hist_anom_CRhist = dsr_hist_clim - ds_hist[REGION].groupby(H_modified_ordinal_day)
ds_proj_anom_CRhist = dsr_hist_clim - ds_proj[REGION].groupby(P_modified_ordinal_day)
...
ds_hist_anom[REGION].cumsum().plot(ax=axes[0,1], label='Daily climatology')
...
ds_proj_anom_Chist[REGION].cumsum().plot(ax=axes[1,0], label='Historical climatology')
ds_proj_anom_Cproj[REGION].cumsum().plot(ax=axes[1,0], label='Projected climatology')
...
ds_proj_anom_Chist[REGION].cumsum().plot(ax=axes[1,1], label='Historical climatology')
...
plt.savefig(FOLDER_project+'results/figures/fig_climatology_proj_'+REGION+'.png')
...
# first subplot is the climatology
ds_hist_clim[REGION].plot(ax=axes[0],label='Daily historical climatology')
...
for year, c in zip(np.arange(start=2006,stop=2065), color):    
     axes[1].plot(ds_proj_anom_CRhist.sel(time=slice(str(year)+'-05-01', str(year+1)+'-04-30')).cumsum(), c=c, linewidth=1, alpha=0.5)
...
plt.savefig(FOLDER_project+'results/figures/fig_combinationv2_'+REGION+'.pdf')

#%%
# =============================================================================
# Adding in a fit to projection data
# =============================================================================
